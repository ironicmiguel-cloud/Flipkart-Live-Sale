...
if (isset($_GET['ajax'])) {
    ...
    if ($act === 'upload' && !empty($_FILES['img']['tmp_name'])) {
        ...
        if (!$destDir) { echo json_encode(['ok'=>false,'err'=>'Invalid slot']); exit; }
        if (!is_dir($destDir)) {
            if (!@mkdir($destDir, 0777, true)) {
                echo json_encode(['ok'=>false,'err'=>'Cannot create folder: '.$destDir.' — set permissions to 777 in your hosting file manager']); exit;
            }
            @chmod($destDir, 0777);
        }
        ...
        // Determine actual extension from uploaded filename (preserve jpg/png/webp/avif)
        $uploadExt = strtolower(pathinfo($_FILES['img']['name'] ?? '', PATHINFO_EXTENSION));
        $ext = in_array($uploadExt, ['jpg','jpeg','png','webp','avif']) ? $uploadExt : 'avif';
        // Re-compute destDir/destFile using the ACTUAL extension of the uploaded file
        [$destDir, $destFile] = getDestPath($cfg, $slot, $imgnum, $ext);
        if (!$destDir) { echo json_encode(['ok'=>false,'err'=>'Invalid slot after ext-fix']); exit; }
        if (!is_dir($destDir)) { @mkdir($destDir, 0777, true); @chmod($destDir, 0777); }
        $base = pathinfo($destFile, PATHINFO_FILENAME);
        foreach (glob("$destDir/$base.*") ?: [] as $old) @unlink($old);
        $dest = "$destDir/$destFile";
        if (!move_uploaded_file($tmp, $dest)) {
            if (!@copy($tmp, $dest)) {
                echo json_encode(['ok'=>false,'err'=>'Write failed at: '.$dest.' — set folder permissions to 777']); exit;
            }
        }
        @chmod($dest, 0644);
        $relUrl = str_replace(BASE_DIR.'/', '', $dest);
        echo json_encode(['ok'=>true,'url'=>"$relUrl?t=".time()]);
        exit;
    }
